...
const DepositPage: React.FC = () => {
  ...
  return (
    <ScrollView className={styles.page} scrollY>
          ...
          <View className={styles.lockItem}>
            ...
            <View className={styles.lockBody}>
              ...
              <Text className={styles.lockMetric}>
                已录 {st.stages_recorded} 阶段 · 均分 {st.assessment_avg} · 项目{st.assess_lock.project_passed ? '已通过' : (st.assess_lock.project_submitted ? '待评审' : '未提交')}
              </Text>
              <View className={styles.progressBar}>
      ...
      {/* 开发调试：一键模拟达标，便于演示退费闭环 */}
      <View className={styles.devBar}>
        <Text className={styles.devTitle}>开发调试（本地演示用）</Text>
        <View className={styles.devBtn} onClick={devStage}><Text>模拟五阶段考核90分+作业+项目</Text></View>
      </View>
    </ScrollView>
  );
};
